...
class Aggregator(object):
    ...
    def aggregate(self, collection):
        ...
        for document in documents:
            ...
            for annotation in impression_passage.annotations:
                ...
                if category != SUPPORT_DEVICES and label in [
                    UNCERTAIN,
                    POSITIVE,
                ]:
                    no_finding = False
                ...
                if (label in [UNCERTAIN, POSITIVE]) and (
                    annotation.text == "chf"
                    or annotation.text == "heart failure"
                ):
    ...
    def aggregate_with_sentence(self, collection):
        labels = []
        documents = collection.documents
        if self.verbose:
            print("Aggregating mentions...")
        ...
        for document in documents:
            ...
            no_finding = True
            for annotation in impression_passage.annotations:
                sents = annotation.infons["sentence"]
                category = annotation.infons[OBSERVATION]
                ...
                if (label in [UNCERTAIN, POSITIVE]) and (
                    ...
                    if CARDIOMEGALY not in label_dict:
                        label_dict[CARDIOMEGALY] = [(UNCERTAIN, sents)]
                    else:
                        label_dict[CARDIOMEGALY].append((UNCERTAIN, sents))

                if category not in label_dict:
                    label_dict[category] = [(label, sents)]
                else:
                    label_dict[category].append((label, sents))

            if no_finding:
                sents = document.passages[0].text
                label_dict[NO_FINDING] = [(POSITIVE, sents)]
