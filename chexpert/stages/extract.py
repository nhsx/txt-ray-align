...
class Extractor(object):
    ...
    def __init__(
        self, mention_phrases_dir, unmention_phrases_dir, verbose=False
    ):
        self.verbose = verbose
        self.observation2mention_phrases = self.load_phrases(
            mention_phrases_dir, "mention"
        )
        self.observation2unmention_phrases = self.load_phrases(
            unmention_phrases_dir, "unmention"
        )
    ...
    def load_phrases(self, phrases_dir, phrases_type):
        ...
        if self.verbose:
            print(
                f"Loading {phrases_type} phrases for "
    ...
    def add_unmention_phrases(self):
        ...
        enlarged_cardiom_mentions = self.observation2mention_phrases[
            ENLARGED_CARDIOMEDIASTINUM
        ]
        positional_phrases = (
            ["over the", "overly the", "in the"],
            ["", " superior", " left", " right"],
        )
    ...
    def overlaps_with_unmention(self, sentence, observation, start, end):
        ...
        unmention_list = self.observation2unmention_phrases.get(
            observation, []
        )
    ...
    def add_match(
        self, impression, sentence, ann_index, phrase, observation, start, end
    ):
        ...
        annotation.infons["sentence"] = sentence.text
        length = end - start
                ...
                for observation, phrases in obs_phrases:
                    for phrase in phrases:
                        ...
                        for match in matches:
                            ...
                            if self.overlaps_with_unmention(
                                sentence, observation, start, end
                            ):
                                continue
