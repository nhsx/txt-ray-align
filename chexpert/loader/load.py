...
class Loader(object):
    ...
    def __init__(self, reports_path, extract_impression=False):
        ...
        self.punctuation_spacer = str.maketrans(
    ...
    def load(self):
        ...
        reports = pd.read_csv(self.reports_path, header=None, names=[REPORTS])[
            REPORTS
        ].tolist()
        ...
        for i, passage in enumerate(document.passages):
            if "title" in passage.infons:
                if passage.infons["title"] == "impression":
                    ...
                    assert (
                        "title" not in next_passage.infons
    ...
    def clean(self, report):
        ...
        corrected_report = re.sub(
            "(?<=[a-zA-Z])/(?=[a-zA-Z])", " or ", corrected_report
        )
