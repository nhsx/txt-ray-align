import torch
import torch.nn as nn
import torch.nn.functional as F
import torchvision.models as models
import pytorch_lightning as pl
import numpy as np
import math
import yaml
import copy

from .model import CLIP
from pl_bolts.optimizers.lr_scheduler import LinearWarmupCosineAnnealingLR
from transformers import AutoTokenizer, AutoModel, BertConfig

# taken from https://pytorch.org/tutorials/beginner/finetuning_torchvision_models_tutorial.html
def set_parameter_requires_grad(model, freeze_encoder):
    if freeze_encoder:
        for param in model.parameters():
            param.requires_grad = False


class VariableSizeBERTModel(nn.Module):
    def __init__(
        self,
        bert_model,
        hidden_size,
        embed_dim,
        freeze_encoder,
        freeze_layers,
        add_projection,
    ):
        """A wrapper for the text encoder for freezing and changing embed size"""

        super().__init__()

        self.add_projection = add_projection

        self.bert_model = bert_model

        if freeze_encoder:
            if freeze_layers == -1:
                set_parameter_requires_grad(self.bert_model, True)
                print("Text model fully frozen:")
            else:
                freeze_layers = range(0, 1 + freeze_layers)
                for name, param in self.bert_model.named_parameters():
                    for layer in freeze_layers:
                        if f"layer.{layer}." in name or "embeddings" in name:
                            param.requires_grad = False
                            print("Text layer frozen:", name)

        # set_parameter_requires_grad(self.bert_model, freeze_encoder)
        if self.add_projection:
            print(f"Adding projection from dim {hidden_size} to {embed_dim}")
            self.linear = nn.Linear(hidden_size, embed_dim)

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None):

        if token_type_ids is not None:
            sequence_output = self.bert_model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
            )[0]
        else:
            sequence_output = self.bert_model(
                input_ids=input_ids, attention_mask=attention_mask
            )[0]
        assert input_ids.shape[0] == sequence_output.shape[0]
        assert input_ids.shape[1] == sequence_output.shape[1]

        sum_embeddings = torch.sum(
            sequence_output * attention_mask.unsqueeze(-1), dim=1
        )
        sum_mask = torch.clamp(
            torch.sum(attention_mask, dim=1, keepdims=True), min=1e-9
        )
        out = sum_embeddings / sum_mask
        if self.add_projection:
            out = self.linear(out)
        return out


class CLIPWrapper(pl.LightningModule):
    def __init__(self, model_name: str, config: dict, minibatch_size: int):
        """A lightning wrapper for a CLIP model as specified in the paper.

        Args:
            model_name (str): A case sensitive visual model name.
            config (dict): A dictionary containing the CLIP instantiation parameters.
        """
        super().__init__()

        self.model_name = model_name
        self.model = CLIP(**config)
        self.minibatch_size = minibatch_size
        self.isViT = "ViT" in self.model_name

        self.automatic_optimization = False

    # Sourced from https://github.com/PyTorchLightning/pytorch-lightning/issues/5449
    @property
    def num_training_steps(self) -> int:
        """Total training steps inferred from datamodule and devices."""
        if self.trainer.max_steps > 0:
            return self.trainer.max_steps

        limit_batches = self.trainer.limit_train_batches
        # this line doesn't work in lightning 1.5.10
        # batches = len(self.train_dataloader())
        batches = len(
            self.trainer._data_connector._train_dataloader_source.dataloader()
        )
        batches = (
            min(batches, limit_batches)
            if isinstance(limit_batches, int)
            else int(limit_batches * batches)
        )

        num_devices = max(1, self.trainer.num_gpus, self.trainer.num_processes)
        if self.trainer.tpu_cores:
            num_devices = max(num_devices, self.trainer.tpu_cores)

        effective_accum = self.trainer.accumulate_grad_batches * num_devices
        return (batches // effective_accum) * self.trainer.max_epochs

    # Training loss: https://github.com/openai/CLIP/issues/83
    # Mini-batching thanks to https://github.com/crowsonkb / https://twitter.com/RiversHaveWings
    # Multi-GPU support: https://github.com/MicPie/clasp
    def training_step(self, train_batch, idx):
        # get optimizers and scheduler
        optimizer = self.optimizers()

        image, text = train_batch

        effective_batch_size = min(self.minibatch_size, len(image))
        n = math.ceil(len(image) // effective_batch_size)
        image_mbs = torch.chunk(image, n)
        text_mbs = torch.chunk(text, n)

        # calculate original statistics
        with torch.no_grad():
            ims = [F.normalize(self.model.encode_image(im), dim=1) for im in image_mbs]
            txt = [F.normalize(self.model.encode_text(t), dim=1) for t in text_mbs]
            # gather from all GPUs
            ims = self.all_gather(torch.cat(ims))
            txt = self.all_gather(torch.cat(txt))

            if len(ims.shape) == 3:
                ims = list(ims)
                txt = list(txt)
            else:
                ims = [ims]
                txt = [txt]

            image_logits = (
                torch.cat(ims) @ torch.cat(txt).t() * self.model.logit_scale.exp()
            )
            ground_truth = (
                torch.arange(len(image_logits)).long().to(image_logits.device)
            )
            loss = (
                F.cross_entropy(image_logits, ground_truth)
                + F.cross_entropy(image_logits.t(), ground_truth)
            ).div(2)
            acc_i = (torch.argmax(image_logits, 1) == ground_truth).sum()
            acc_t = (torch.argmax(image_logits, 0) == ground_truth).sum()
            self.log_dict(
                {
                    "loss": loss / len(ims),
                    "acc": (acc_i + acc_t) / 2 / len(image) / len(ims),
                },
                prog_bar=True,
            )

        if isinstance(optimizer, list):
            optimizer = optimizer[0]
        optimizer.zero_grad()

        # image loss
        for j, mb in enumerate(image_mbs):
            images_tmp = copy.deepcopy(ims)
            images_tmp[self.global_rank][
                j * self.minibatch_size : (j + 1) * self.minibatch_size
            ] = F.normalize(self.model.encode_image(mb), dim=1)
            image_logits = (
                torch.cat(images_tmp)
                @ torch.cat(txt).t()
                * self.model.logit_scale.exp()
            )
            ground_truth = (
                torch.arange(len(image_logits)).long().to(image_logits.device)
            )
            loss = (
                F.cross_entropy(image_logits, ground_truth)
                + F.cross_entropy(image_logits.t(), ground_truth)
            ) / 2
            self.manual_backward(loss)

        # text loss
        for j, mb in enumerate(text_mbs):
            text_tmp = copy.deepcopy(txt)
            text_tmp[self.global_rank][
                j * self.minibatch_size : (j + 1) * self.minibatch_size
            ] = F.normalize(self.model.encode_text(mb), dim=1)
            image_logits = (
                torch.cat(ims) @ torch.cat(text_tmp).t() * self.model.logit_scale.exp()
            )
            loss = (
                F.cross_entropy(image_logits, ground_truth)
                + F.cross_entropy(image_logits.t(), ground_truth)
            ) / 2
            self.manual_backward(loss)

        optimizer.step()
        lr_scheduler = self.lr_schedulers()
        lr_scheduler.step()
        self.model.logit_scale.data.clamp_(-np.log(100), np.log(100))

    def validation_step(self, val_batch, idx):
        image, text = val_batch
        image_logits, text_logits = self.forward(image, text)
        ground_truth = torch.arange(len(image_logits)).long().to(image_logits.device)
        loss = (
            F.cross_entropy(image_logits, ground_truth)
            + F.cross_entropy(text_logits, ground_truth)
        ).div(2)
        self.log("val_loss", loss)

    def configure_optimizers(self):
        lr = {
            "RN50": 5e-4,
            "RN101": 5e-4,
            "RN50x4": 5e-4,
            "RN50x16": 4e-4,
            "RN50x64": 3.6e-4,
            "ViT-B/32": 5e-4,
            "ViT-B/16": 5e-4,
            "ViT-L/14": 4e-4,
            "ViT-L/14-336px": 2e-5,
        }[self.model_name]

        optimizer = torch.optim.AdamW(
            self.model.parameters(),
            lr=lr,
            betas=(0.9, 0.98 if self.isViT else 0.999),
            eps=1e-6 if self.isViT else 1e-8,
            weight_decay=0.2,
        )

        lr_scheduler = LinearWarmupCosineAnnealingLR(
            optimizer, warmup_epochs=2000, max_epochs=self.num_training_steps
        )

        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler}


class CustomCLIPWrapper(CLIPWrapper):
    def __init__(
        self,
        image_encoder,
        text_encoder,
        minibatch_size,
        using_clip=False,
        optimizer="AdamW",
        lr=3e-3,
        lr_img=3e-3,
        lr_txt=3e-3,
        kl_coeff=1.0,
        warmup_epochs=1,
        use_teacher=False,
    ):
        with open("models/configs/RN.yaml") as fin:
            config = yaml.safe_load(fin)["RN50"]
        super().__init__("RN50", config, minibatch_size)
        self.save_hyperparameters(ignore=["image_encoder", "text_encoder"])
        del self.model.visual
        del self.model.transformer
        self.model.visual = image_encoder
        self.model.transformer = text_encoder
        self.using_clip = using_clip
        self.optimizer = optimizer
        self.lr = lr
        self.lr_img = lr_img
        self.lr_txt = lr_txt
        self.warmup_epochs = warmup_epochs
        self.sink_temp = nn.Parameter(
            torch.ones([]) * np.log(1 / 0.07)
        )  # from the CLIP paper

        # init self-distillation model
        self.teacher = None
        if use_teacher:
            self.teacher = copy.deepcopy(self.model)
        self.kl_coeff = kl_coeff

    def finish_clip_init(self, clp):
        self.model.token_embedding = clp.token_embedding
        self.model.ln_final = clp.ln_final
        self.model.text_projection = clp.text_projection
        if self.teacher is not None:
            self.teacher = copy.deepcopy(self.model)

    def training_step(self, train_batch, idx):
        # get optimizers and scheduler
        optimizer = self.optimizers()

        image, text = train_batch
        effective_batch_size = min(self.minibatch_size, len(image))
        n = math.ceil(len(image) // effective_batch_size)

        image_mbs = torch.chunk(image, n)

        text_mbs = None
        if self.using_clip:
            text_mbs = torch.chunk(text, n)
        else:
            # adjust embedding dictionaries - one for every microbatch
            text_mbs_ids = torch.chunk(torch.arange(len(image)), n)
            text_mbs = []
            for s in text_mbs_ids:
                d = {}
                for key in list(text.keys()):
                    d[key] = text[key][s]
                text_mbs.append(d)

        # calculate original statistics
        with torch.no_grad():
            ims = [F.normalize(self.model.encode_image(im), dim=1) for im in image_mbs]
            txt = [F.normalize(self.encode_text(t), dim=1) for t in text_mbs]
            # gather from all GPUs
            ims = self.all_gather(torch.cat(ims))
            txt = self.all_gather(torch.cat(txt))

            if len(ims.shape) == 3:
                ims = list(ims)
                txt = list(txt)
            else:
                ims = [ims]
                txt = [txt]

            image_logits_notemp = torch.cat(ims) @ torch.cat(txt).t()
            image_logits = image_logits_notemp * self.model.logit_scale.exp()
            ground_truth = (
                torch.arange(len(image_logits)).long().to(image_logits.device)
            )
            loss = (
                F.cross_entropy(image_logits, ground_truth)
                + F.cross_entropy(image_logits.t(), ground_truth)
            ).div(2)
            acc_i = (torch.argmax(image_logits, 1) == ground_truth).sum()
            acc_t = (torch.argmax(image_logits, 0) == ground_truth).sum()
            # calculate teacher
            if self.teacher is not None:
                teacher_ims = [
                    F.normalize(self.teacher.encode_image(im), dim=1)
                    for im in image_mbs
                ]
                teacher_txt = [
                    F.normalize(self.encode_text(t, teacher=True), dim=1)
                    for t in text_mbs
                ]

                teacher_ims = self.all_gather(torch.cat(teacher_ims))
                teacher_txt = self.all_gather(torch.cat(teacher_txt))

                if len(teacher_ims.shape) == 3:
                    teacher_ims = list(teacher_ims)
                    teacher_txt = list(teacher_txt)
                else:
                    teacher_ims = [teacher_ims]
                    teacher_txt = [teacher_txt]

                sim_ii, sim_tt, sim_it, sim_ti = self.compute_similarities(
                    torch.cat(teacher_ims), torch.cat(teacher_txt)
                )

                # optimal transport
                img_cost = -(sim_ii + sim_tt + sim_it)
                txt_cost = -(sim_ii + sim_tt + sim_ti)
                img_target = self.sinkhorn(img_cost)
                txt_target = self.sinkhorn(txt_cost)
                loss += (
                    (
                        F.kl_div(
                            F.log_softmax(image_logits_notemp * self.sink_temp, dim=-1),
                            img_target,
                            reduction="batchmean",
                        )
                        + F.kl_div(
                            F.log_softmax(
                                image_logits_notemp.t() * self.sink_temp, dim=-1
                            ),
                            txt_target,
                            reduction="batchmean",
                        )
                    )
                    / 2
                    * self.kl_coeff
                )
            self.log_dict(
                {
                    "loss": loss / len(ims),
                    "acc": (acc_i + acc_t) / 2 / len(image) / len(ims),
                    "lr": self.lr_schedulers().get_last_lr()[0],
                    "lr_img": self.lr_schedulers().get_last_lr()[1],
                    "lr_txt": self.lr_schedulers().get_last_lr()[2],
                },
                prog_bar=True,
            )

        if isinstance(optimizer, list):
            optimizer = optimizer[0]
        optimizer.zero_grad()

        # image loss
        for j, mb in enumerate(image_mbs):
            images_tmp = copy.deepcopy(ims)
            images_tmp[self.global_rank][
                j * self.minibatch_size : (j + 1) * self.minibatch_size
            ] = F.normalize(self.model.encode_image(mb), dim=1)
            image_logits_notemp = torch.cat(images_tmp) @ torch.cat(txt).t()
            image_logits = image_logits_notemp * self.model.logit_scale.exp()
            loss = (
                F.cross_entropy(image_logits, ground_truth)
                + F.cross_entropy(image_logits.t(), ground_truth)
            ) / 2
            if self.teacher is not None:
                loss += (
                    (
                        F.kl_div(
                            F.log_softmax(image_logits_notemp * self.sink_temp, dim=-1),
                            img_target,
                            reduction="batchmean",
                        )
                        + F.kl_div(
                            F.log_softmax(
                                image_logits_notemp.t() * self.sink_temp, dim=-1
                            ),
                            txt_target,
                            reduction="batchmean",
                        )
                    )
                    / 2
                    * self.kl_coeff
                )
            self.manual_backward(loss)

        # text loss
        for j, mb in enumerate(text_mbs):
            text_tmp = copy.deepcopy(txt)
            text_tmp[self.global_rank][
                j * self.minibatch_size : (j + 1) * self.minibatch_size
            ] = F.normalize(self.encode_text(mb), dim=1)
            image_logits_notemp = torch.cat(ims) @ torch.cat(text_tmp).t()
            image_logits = image_logits_notemp * self.model.logit_scale.exp()
            loss = (
                F.cross_entropy(image_logits, ground_truth)
                + F.cross_entropy(image_logits.t(), ground_truth)
            ) / 2
            if self.teacher is not None:
                loss += (
                    (
                        F.kl_div(
                            F.log_softmax(image_logits_notemp * self.sink_temp, dim=-1),
                            img_target,
                            reduction="batchmean",
                        )
                        + F.kl_div(
                            F.log_softmax(
                                image_logits_notemp.t() * self.sink_temp, dim=-1
                            ),
                            txt_target,
                            reduction="batchmean",
                        )
                    )
                    / 2
                    * self.kl_coeff
                )
            self.manual_backward(loss)

        optimizer.step()
        lr_scheduler = self.lr_schedulers()
        lr_scheduler.step()
        self.model.logit_scale.data.clamp_(-np.log(100), np.log(100))
        self.sink_temp.data.clamp_(-np.log(100), np.log(100))
        if self.teacher is not None:
            self.update_teacher()

    def encode_text(self, inputs, teacher=False):
        embeddings = None
        if self.using_clip:
            inference = self.teacher if teacher else self.model

            x = inference.token_embedding(inputs).type(
                self.dtype
            )  # [batch_size, n_ctx, d_model]

            x = x + inference.positional_embedding.type(self.dtype)
            x = x.permute(1, 0, 2)  # NLD -> LND
            x = inference.transformer(x)
            x = x.permute(1, 0, 2)  # LND -> NLD
            x = inference.ln_final(x).type(self.dtype)

            # x.shape = [batch_size, n_ctx, transformer.width]
            # take features from the eot embedding (eot_token is the highest number in each sequence)
            x = (
                x[torch.arange(x.shape[0]), inputs.argmax(dim=-1)]
                @ self.model.text_projection
            )
            embeddings = x
        else:
            embeddings = (
                self.teacher.transformer(**inputs)
                if teacher
                else self.model.transformer(**inputs)
            )
        return embeddings

    def compute_similarities(self, I_emb, T_emb):
        sim_ii, sim_tt = I_emb @ I_emb.t(), T_emb @ T_emb.t()
        sim_it, sim_ti = I_emb @ T_emb.t(), T_emb @ I_emb.t()
        return sim_ii, sim_tt, sim_it, sim_ti

    def update_teacher(self):
        for teacher, student in zip(self.teacher.parameters(), self.model.parameters()):
            teacher.data.copy_(self.ema(student.data, teacher.data))

    def ema(self, s, t):
        return s * (1 - 0.999) + t * 0.999

    def forward(self, images, text):
        logits = (
            F.normalize(self.model.encode_image(images), dim=1)
            @ F.normalize(self.encode_text(text), dim=1).t()
            * self.model.logit_scale.exp()
        )
        return logits, logits.t()

    # Sourced from: https://github.com/facebookresearch/swav/blob/5e073db0cc69dea22aa75e92bfdd75011e888f28/main_swav.py#L354
    def sinkhorn(self, out):
        Q = torch.exp(
            out / 0.05
        ).t()  # Q is K-by-B for consistency with notations from our paper
        B = Q.shape[1]  # number of samples to assign
        K = Q.shape[0]  # how many prototypes

        # make the matrix sums to 1
        sum_Q = torch.sum(Q)
        Q /= sum_Q

        for it in range(3):
            # normalize each row: total weight per prototype must be 1/K
            sum_of_rows = torch.sum(Q, dim=1, keepdim=True)
            Q /= sum_of_rows
            Q /= K

            # normalize each column: total weight per sample must be 1/B
            Q /= torch.sum(Q, dim=0, keepdim=True)
            Q /= B

        Q *= B  # the colomns must sum to 1 so that Q is an assignment
        return Q.t()

    def configure_optimizers(self):
        lr_img = self.lr_img if self.lr_img is not None else self.lr
        lr_txt = self.lr_txt if self.lr_txt is not None else self.lr

        rest = lambda n, p: "visual" not in n and "transformer" not in n
        named_parameters = list(self.named_parameters())
        rest_params = [p for n, p in named_parameters if rest(n, p) and p.requires_grad]

        if self.optimizer == "AdamW":
            optimizer = torch.optim.AdamW(
                [
                    {"params": rest_params, "lr": self.lr},
                    {"params": self.model.visual.parameters(), "lr": lr_img},
                    {"params": self.model.transformer.parameters(), "lr": lr_txt},
                ],
                weight_decay=1e-6,
            )
        elif self.optimizer == "SGD":
            optimizer = torch.optim.SGD(
                [
                    {"params": rest_params, "lr": self.lr},
                    {"params": self.model.visual.parameters(), "lr": lr_img},
                    {"params": self.model.transformer.parameters(), "lr": lr_txt},
                ],
                momentum=0.9,
            )
        else:
            print("Please choose AdamW or SGD, exiting...")
            exit(1)

        print("Optimiser ", optimizer)
        print("Num training steps ", self.num_training_steps)
        print(
            "Num steps per epoch ", self.num_training_steps // self.trainer.max_epochs
        )

        lr_scheduler = LinearWarmupCosineAnnealingLR(
            optimizer,
            warmup_epochs=self.warmup_epochs
            * (self.num_training_steps // self.trainer.max_epochs),
            max_epochs=self.num_training_steps,
        )

        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler}


# adapted from https://pytorch.org/tutorials/beginner/finetuning_torchvision_models_tutorial.html
def init_img_model(model_name, embed_dim, freeze_encoder=False, use_pretrained=True):
    # Initialize these variables which will be set in this if statement. Each of these
    #   variables is model specific.
    model_ft = None
    input_size = 0

    if model_name == "resnet18":
        """Resnet18"""
        model_ft = models.resnet18(pretrained=use_pretrained)
        set_parameter_requires_grad(model_ft, freeze_encoder)
        num_ftrs = model_ft.fc.in_features
        model_ft.fc = nn.Linear(num_ftrs, embed_dim)
        input_size = 224

    elif model_name == "resnet50":
        """Resnet50"""
        model_ft = models.resnet50(pretrained=use_pretrained)
        set_parameter_requires_grad(model_ft, freeze_encoder)
        num_ftrs = model_ft.fc.in_features
        model_ft.fc = nn.Linear(num_ftrs, embed_dim)
        input_size = 224

    elif model_name == "efficientnet_b0":
        """efficientnet_b0"""
        model_ft = models.efficientnet_b0(pretrained=use_pretrained)
        set_parameter_requires_grad(model_ft, freeze_encoder)
        num_ftrs = model_ft.classifier[1].in_features
        model_ft.classifier[1] = nn.Linear(num_ftrs, embed_dim)
        input_size = 224

    else:
        print("Invalid model name, exiting...")
        exit()

    return model_ft, input_size


def init_txt_model(
    model_name,
    embed_dim,
    freeze_encoder=False,
    freeze_layers=-1,
    add_projection=False,
    local_files_only=False,
):

    model_dict = {
        "declutr": "johngiorgi/declutr-sci-base",
        "bioclinicalbert": "emilyalsentzer/Bio_ClinicalBERT",
        "distilbert": "distilbert-base-uncased",
    }

    if model_name in model_dict.keys():
        model_name = model_dict[model_name]
        tokenizer = AutoTokenizer.from_pretrained(
            model_name, local_files_only=local_files_only
        )
        txt_encoder = AutoModel.from_pretrained(
            model_name, local_files_only=local_files_only
        )
        hidden_size = BertConfig(model_name).hidden_size
        txt_encoder = VariableSizeBERTModel(
            txt_encoder,
            hidden_size,
            embed_dim,
            freeze_encoder,
            freeze_layers,
            add_projection,
        )
    else:
        print(f"Choose from {model_dict.keys()}, exiting...")
        exit(1)

    return txt_encoder, tokenizer
