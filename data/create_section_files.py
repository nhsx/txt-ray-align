# Liberally borrowed from https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv-cxr/txt/create_section_files.py
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Concatenate last paragraph, impression and findings.",
)
parser.add_argument(
    ...
    if len(patient_studies) > 0:
        ...
        if args.no_split:
            ...
            with open(output_path / f"mimic_cxr_sections.csv", "w") as fp:
                ...
                if args.concat:
                    for row in study_sections:
                        # concat last_paragraph, impressions and findings
                        csvwriter.writerow(
                            [row[0], "".join(filter(None, [row[1], row[2], row[3]]))]
                        )
                else:
            ...
            while n < len(patient_studies):
                ...
                with open(output_path / f"mimic_cxr_{n_fn:02d}.csv", "w") as fp:
                    csvwriter = csv.writer(fp)
                    if args.concat:
                        for row in study_sections[n : n + jmp]:
                            # concat last_paragraph, impressions and findings
                            csvwriter.writerow(
                                [
                                    row[0],
                                    "".join(filter(None, [row[1], row[2], row[3]])),
                                ]
                            )
                    else:
